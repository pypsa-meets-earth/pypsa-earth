...
1. Build a cutout with build_cutout rule
   - snapshots correspond to a dataframe of interest

   The standard time frames used in the default configuration for a full-scale and tutorial model are as follows:
       ```
       # full-scale cutout
       snapshots:
         start: "2013-01-01"
...
3. Compress the cutout and upload a file:
   ...
   - change the archive name to a meaningful. For example:
   - `bundle_cutouts_{region}.zip`
   - `bundle_cutouts_tutorial_{region}.zip`
   -
