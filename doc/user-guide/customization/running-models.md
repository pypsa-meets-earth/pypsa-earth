...
PyPSA-Earth utilizes the `snakemake` workflow management tool inherited from PyPSA-Eur.
...
## Execute Different Parts of the Workflow

The snakemake included in the `pypsa-earth` conda environment can be used to execute any custom rule with the following command:

```bash
snakemake <your custom rule>
```
...
For example, power grid topology may be extracted and cleaned with the following command which refers to the script name:

```bash
snakemake -j 1 clean_osm_data
```

Solar profile for the requested area may be calculated using the output name:

```bash
snakemake -j 1 resources/renewable_profiles/profile_solar.nc
```
...
`glpk` can solve the network with low temporal and spatial resolution. To make a full model run, it is advised to use `CPLEX`, `Gurobi`, or open-source [HiGHS](https://highs.dev/).
