...
Another great resource for cost assumptions is the `cost database from the Danish Energy Agency](https://ens.dk/en/our-services/projections-and-models/technology-data).
...
Some cost assumptions (e.g. marginal cost and capital cost) can be directly overwritten in the `config.yaml`` (cf. Section  [costs_cf]  in [config]).
