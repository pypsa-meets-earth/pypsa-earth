...
Reload the solved network in `analyze_kz.ipynb`. The OSM rebuild changes line capacities and topology, so check whether [Part 6](6-network-topology.md#step-7-verify-the-fix)'s zero-shedding result still holds now that lines carry realistic, non-extendable ratings, and that demand is near the KEGOC target:
