...
rule prepare_transport_data_input:
    output:
        transport_data_input="resources/" + RDIR + "transport_data.csv",
    script:
...
if not config["custom_data"]["gas_network"]:
    ...
    rule prepare_gas_network:
        ...
        input:
            ...
            + "bus_regions/regions_onshore_elec_s{simpl}_{clusters}.geojson",
        output:
            clustered_gas_network="resources/" + RDIR + "gas_networks/gas_network_elec_s{simpl}_{clusters}.csv",
...
rule prepare_sector_network:
    ...
    input:
        ...
        nodal_energy_totals="resources/" + RDIR + "demand/heat/nodal_energy_heat_totals_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        transport="resources/" + RDIR + "demand/transport_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        avail_profile="resources/" + RDIR + "pattern_profiles/avail_profile_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        dsm_profile="resources/" + RDIR + "pattern_profiles/dsm_profile_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        nodal_transport_data="resources/" + RDIR + "demand/nodal_transport_data_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        overrides="data/override_component_attrs",
        clustered_pop_layout="resources/" + RDIR + "population_shares/pop_layout_elec_s{simpl}_{clusters}_{planning_horizons}.csv",
        industrial_demand="resources/" + RDIR + "demand/industrial_energy_demand_per_node_elec_s{simpl}_{clusters}_{planning_horizons}_{demand}.csv",
        energy_totals="data/energy_totals_{demand}_{planning_horizons}.csv",
        airports="data/airports.csv",
        ports="data/ports.csv",
        heat_demand="resources/" + RDIR + "demand/heat/heat_demand_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        ashp_cop="resources/" + RDIR + "demand/heat/ashp_cop_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        gshp_cop="resources/" + RDIR + "demand/heat/gshp_cop_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        solar_thermal="resources/" + RDIR + "demand/heat/solar_thermal_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        district_heat_share="resources/" + RDIR + "demand/heat/district_heat_share_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
        ...
        pipelines=(
            ...
            else "resources/" + RDIR + "gas_networks/gas_network_elec_s{simpl}_{clusters}.csv"
        ),
    output:
        RESDIR
        + "prenetworks/elec_s{simpl}_{clusters}_ec_l{ll}_{opts}_{sopts}_{planning_horizons}_{discountrate}_{demand}.nc",
...
rule build_ship_profile:
    ...
    output:
        ship_profile="resources/" + RDIR + "ship_profile_{h2export}TWh.csv",
    script:
...
rule override_respot:
    ...
    input:
        **{
            f"custom_res_pot_{tech}_{planning_horizons}_{discountrate}": "resources/" + RDIR + f"custom_renewables/{tech}_{planning_horizons}_{discountrate}_potential.csv"
        ...
        **{
            f"custom_res_ins_{tech}_{planning_horizons}_{discountrate}": "resources/" + RDIR + f"custom_renewables/{tech}_{planning_horizons}_{discountrate}_installable.csv"
            for tech in config["custom_data"]["renewables"]
            for discountrate in config["costs"]["discountrate"]
            for planning_horizons in config["scenario"]["planning_horizons"]
        },
...
rule prepare_transport_data:
    input:
        ...
        clustered_pop_layout="resources/" + RDIR + "population_shares/pop_layout_elec_s{simpl}_{clusters}_{planning_horizons}.csv",
        temp_air_total="resources/" + RDIR + "temperatures/temp_air_total_elec_s{simpl}_{clusters}_{planning_horizons}.nc",
    output:
...
rule build_cop_profiles:
    ...
    output:
        ...
        cop_soil_rural="resources/" + RDIR + "cops/cop_soil_rural_elec_s{simpl}_{clusters}_{planning_horizons}.nc",
        cop_soil_urban="resources/" + RDIR + "cops/cop_soil_urban_elec_s{simpl}_{clusters}_{planning_horizons}.nc",
        cop_air_total="resources/" + RDIR + "cops/cop_air_total_elec_s{simpl}_{clusters}_{planning_horizons}.nc",
        cop_air_rural="resources/" + RDIR + "cops/cop_air_rural_elec_s{simpl}_{clusters}_{planning_horizons}.nc",
        cop_air_urban="resources/" + RDIR + "cops/cop_air_urban_elec_s{simpl}_{clusters}_{planning_horizons}.nc",
    resources:
        mem_mb=20000,
    benchmark:
        "benchmarks/" + RDIR + "build_cop_profiles/s{simpl}_{clusters}_{planning_horizons}"
    script:
...
rule build_solar_thermal_profiles:
    ...
    input:
        ...
        pop_layout_rural="resources/" + RDIR + "population_shares/pop_layout_rural_{planning_horizons}.nc",
        regions_onshore="resources/"
        + RDIR
        + "bus_regions/regions_onshore_elec_s{simpl}_{clusters}.geojson",
        cutout="cutouts/"
    ...
    resources:
        mem_mb=20000,
    benchmark:
        "benchmarks/" + RDIR + "build_solar_thermal_profiles/s{simpl}_{clusters}_{planning_horizons}"
    script:
...
rule build_population_layouts:
    ...
    resources:
        mem_mb=20000,
    benchmark:
        "benchmarks/" + RDIR + "build_population_layouts_{planning_horizons}"
    threads: 8
    script:
...
rule move_hardcoded_files_temp:
    ...
    output:
        "resources/" + RDIR + "energy_totals.csv",
    shell:
...
rule build_clustered_population_layouts:
    ...
    output:
        clustered_pop_layout="resources/" + RDIR + "population_shares/pop_layout_elec_s{simpl}_{clusters}_{planning_horizons}.csv",
        clustered_gdp_layout="resources/" + RDIR + "gdp_shares/gdp_layout_elec_s{simpl}_{clusters}_{planning_horizons}.csv",
    resources:
        mem_mb=10000,
    benchmark:
        "benchmarks/" + RDIR + "build_clustered_population_layouts/s{simpl}_{clusters}_{planning_horizons}"
    script:
...
rule build_heat_demand:
    ...
    resources:
        mem_mb=20000,
    benchmark:
        "benchmarks/" + RDIR + "build_heat_demand/s{simpl}_{clusters}_{planning_horizons}"
    script:
...
rule build_temperature_profiles:
    ...
    resources:
        mem_mb=20000,
    benchmark:
        "benchmarks/" + RDIR + "build_temperature_profiles/s{simpl}_{clusters}_{planning_horizons}"
    script:
...
rule build_industrial_distribution_key:  #default data
    ...
    input:
        ...
        + "bus_regions/regions_onshore_elec_s{simpl}_{clusters}.geojson",
    output:
        industrial_distribution_key="resources/" + RDIR + "demand/industrial_distribution_key_elec_s{simpl}_{clusters}_{planning_horizons}.csv",
    threads: 1
    resources:
        mem_mb=1000,
    benchmark:
        "benchmarks/" + RDIR + "build_industrial_distribution_key_elec_s{simpl}_{clusters}_{planning_horizons}"
    script:
...
rule build_base_industry_totals:  #default data
    ...
    output:
        base_industry_totals="resources/" + RDIR + "demand/base_industry_totals_{planning_horizons}_{demand}.csv",
    threads: 1
    resources:
        mem_mb=1000,
    benchmark:
        "benchmarks/" + RDIR + "build_base_industry_totals_{planning_horizons}_{demand}"
    script:
...
rule build_industry_demand:  #default data
    ...
    output:
        industrial_energy_demand_per_node="resources/" + RDIR + "demand/industrial_energy_demand_per_node_elec_s{simpl}_{clusters}_{planning_horizons}_{demand}.csv",
    threads: 1
    resources:
        mem_mb=1000,
    benchmark:
        "benchmarks/" + RDIR + "industrial_energy_demand_per_node_elec_s{simpl}_{clusters}_{planning_horizons}_{demand}.csv"
    script:
...
rule build_existing_heating_distribution:
    ...
    input:
        ...
        clustered_pop_layout="resources/" + RDIR + "population_shares/pop_layout_elec_s{simpl}_{clusters}_{planning_horizons}.csv",
        clustered_pop_energy_layout="resources/" + RDIR + "demand/heat/nodal_energy_heat_totals_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",  #"resources/population_shares/pop_weighted_energy_totals_s{simpl}_{clusters}.csv",
        district_heat_share="resources/" + RDIR + "demand/heat/district_heat_share_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
    output:
        existing_heating_distribution="resources/" + RDIR + "heating/existing_heating_distribution_{demand}_s{simpl}_{clusters}_{planning_horizons}.csv",
    threads: 1
    resources:
        mem_mb=2000,
    log:
