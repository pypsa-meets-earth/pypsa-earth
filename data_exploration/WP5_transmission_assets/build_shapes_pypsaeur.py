...
def countries():
    ...
    df = df.loc[
        df.name.isin(cntries) & ((df["scalerank"] == 0) | (df["scalerank"] == 5))
    ]
...
def nuts3(country_shapes):
    ...
    pop = pd.read_table(
        snakemake.input.nuts3pop, na_values=[":"], delimiter=" ?\t", engine="python"
    )
    pop = (
        pop.set_index(
    ...
    gdp = pd.read_table(
        snakemake.input.nuts3gdp, na_values=[":"], delimiter=" ?\t", engine="python"
    )
    gdp = (
        gdp.set_index(
