...
def countries():
    ...
    df["name"] = reduce(lambda x, y: x.fillna(y), fieldnames,
                        next(fieldnames)).str[0:2]
...
def eez(country_shapes):
    ...
    df = df.loc[df["ISO_3digit"].isin([
        _get_country("alpha_3", alpha_2=c)
        for c in snakemake.config["countries"]
    ])]
    ...
    s = gpd.GeoSeries({
        k: v
        for k, v in s.iteritems() if v.distance(country_shapes[k]) < 1e-3
    })
    s.index.name = "name"
    return s
...
def nuts3(country_shapes):
    ...
    df = df.rename(columns={"NUTS_ID": "id"})[["id",
                                               "geometry"]].set_index("id")

    pop = pd.read_table(snakemake.input.nuts3pop,
                        na_values=[":"],
                        delimiter=" ?\t",
                        engine="python")
    pop = (pop.set_index(
        pd.MultiIndex.from_tuples(
            pop.pop("unit,geo\\time").str.split(","))).loc["THS"].applymap(
                lambda x: pd.to_numeric(x, errors="coerce")).fillna(
                    method="bfill", axis=1))["2014"]

    gdp = pd.read_table(snakemake.input.nuts3gdp,
                        na_values=[":"],
                        delimiter=" ?\t",
                        engine="python")
    gdp = (gdp.set_index(
        pd.MultiIndex.from_tuples(
            gdp.pop("unit,geo\\time").str.split(","))).loc["EUR_HAB"].applymap(
                lambda x: pd.to_numeric(x, errors="coerce")).fillna(
                    method="bfill", axis=1))["2014"]
    ...
    gdp = gdp.append(
        pd.to_numeric(
            swiss.loc["Gross domestic product per capita in Swiss francs",
                      "CH040":]))

    df = df.join(pd.DataFrame(dict(pop=pop, gdp=gdp)))

    df["country"] = df.index.to_series().str[:2].replace(dict(UK="GB",
                                                              EL="GR"))
