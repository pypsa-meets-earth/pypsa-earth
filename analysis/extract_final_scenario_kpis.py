...
def get_co2_emissions(n, w):
    ...
    for carrier, idx in (
        ...
        efficiency = n.generators.loc[
            idx,
            "efficiency"
        ]

        fuel_input = dispatch.div(
            efficiency,
            axis=1
        )
...
def extract_scenario(name, path):
    ...
    coal_gen = get_generation(
        n, "coal", w
    )
    ccgt_gen = get_generation(
        n, "CCGT", w
    )
    ocgt_gen = get_generation(
        n, "OCGT", w
    )
    lignite_gen = get_generation(
        n, "lignite", w
    )
    oil_gen = get_generation(
        n, "oil", w
    )
    solar_gen = get_generation(
        n, "solar", w
    )
    wind_gen = get_generation(
        n, "onwind", w
    )
    ror_gen = get_generation(
        n, "ror", w
    )
    load_shedding = get_generation(
        n, "load shedding", w
    )
    ...
    row = {
        ...
        "CO2 limit utilisation [%]":
            co2_utilisation,

        "CO2 constraint dual mu [EUR/tCO2]":
            co2_mu,

        "CO2 shadow-price magnitude [EUR/tCO2]":
            (
                abs(co2_mu)
                if np.isfinite(co2_mu)
                else np.nan
            ),
        ...
        "Renewable share of gross generation [%]":
            renewable_share,
...
COMPARISON_OUTPUT = Path(
    "results/scenarios/final_scenario_comparison.csv"
)

s0 = df.loc[
    df["Scenario"] == "S0_2045_Reference"
].iloc[0]

s1 = df.loc[
    df["Scenario"] == "S1_2045_NetZero"
].iloc[0]
...
if is_percentage_kpi:
    ...
    if (
        pd.notna(value_s0)
        and pd.notna(value_s1)
        and abs(value_s0) > 1e-4
    ):
        relative_change = (
            100
            * (value_s1 - value_s0)
