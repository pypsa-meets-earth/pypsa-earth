...
def reservoir_capacity_gw(n):
    ...
    mask = (
        c.str.contains("hydro")
        |
        c.str.contains("reservoir")
    )
...
def co2_for_generator_group_mt(
    ...
    for name in idx:
        ...
        if (
            carrier
            not in n.carriers.index
        ):
            continue

        if (
            "co2_emissions"
            not in n.carriers.columns
        ):
            continue
...
def battery_metrics(n):
    ...
    if n.stores.empty:
        ...
        mask = (
            c.str.contains("battery")
            |
            names.str.contains("battery")
        )
...
def btc_metrics(
    ...
    result = {
        ...
        "BTC operating hours [h]":
            0.0,

        "BTC full-power hours [h]":
            0.0,

        "BTC modeled value [EUR bn]":
            0.0,

        "BTC weighted electricity price [EUR/MWh]":
            np.nan,
    }
...
def h2_metrics(
    ...
    result = {
        ...
        "PEM variable cost [EUR bn]":
            0.0,

        "PEM weighted electricity price [EUR/MWh]":
            np.nan,
    }
...
def extract_scenario(
    ...
    row = {
        ...
        "Raw optimization objective [EUR bn]":
            raw_objective_bn,
        ...
        "Renewable share of gross primary generation [%]":
            renewable_share,
        ...
        "Unweighted marginal price [EUR/MWh]":
            unweighted_price,
...
for scenario in [
    ...
    check_n = pypsa.Network(
        resolved_paths[scenario]
    )
    ...
    if abs(
        ...
        raise AssertionError(
            f"{scenario}: BTC capacity is "
            f"{btc_capacity} GW, expected 1.0 GW."
        )
...
for scenario in [
    ...
    if abs(btc_energy) > 1e-9:
        raise AssertionError(
            f"{scenario}: unexpected BTC consumption "
            f"{btc_energy} TWh/a."
        )
...
for scenario in [
    ...
    if abs(
        ...
        raise AssertionError(
            f"{scenario}: H2 production is "
            f"{h2_kt} kt/a, expected 100 kt/a."
        )


print(
    "\\nFinal flexible-consumer representation "
    "invariants: PASSED"
)
...
df[
    "Incremental CO2 vs baseline [MtCO2/a]"
] = np.nan
...
PAIRS = {
    "S2-S0 Reference BTC effect":
        ("S2", "S0"),

    "S3-S0 Reference H2 effect":
        ("S3", "S0"),

    "S4-S1 NetZero BTC effect":
        ("S4", "S1"),

    "S5-S1 NetZero H2 effect":
        ("S5", "S1"),

    "S6-S1 NetZero combined effect":
        ("S6", "S1"),
}
...
for comparison, (
    ...
    for kpi in PAIR_KPIS:
        ...
        scenario_value = df.loc[
            scenario,
            kpi
        ]

        baseline_value = df.loc[
            baseline,
            kpi
        ]
...
kpi_file = (
    OUT
    / "final_S0_S6_flexible_kpis.csv"
)

pair_file = (
    OUT
    / "final_S0_S6_pairwise_changes.csv"
)

interaction_file = (
    OUT
    / "final_S0_S6_interaction_terms.csv"
)

competition_file = (
    OUT
    / "final_S4_S6_BTC_H2_competition.csv"
)
