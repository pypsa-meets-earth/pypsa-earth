...
if __name__ == "__main__":
    ...
    industry_sector_ratios = pd.read_csv(snakemake.input.industry_sector_ratios, index_col=0)
    ...
    production_tod = pd.read_csv(prod_tod_path, header=0, index_col=0).filter(snakemake.config['countries'], axis=0)
    production_tod.at['MA', 'Integrated steelworks'] = 1e5

    nodal_production_tod = country_to_nodal(production_tod, dist_keys)
    nodal_production_tod = nodal_production_tod.reindex(columns=list(industry_sector_ratios.columns), fill_value=0)
    ...
    nodal_df.to_csv(snakemake.output.industrial_energy_demand_per_node, float_format="%.2f")
