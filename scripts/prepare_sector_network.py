...
def add_industry(n, costs):
    ...
    all_navigation = ["total international navigation",
    ...
    all_navigation = ["total international navigation",
                      "total domestic navigation"]
    nodal_energy_totals = pd.DataFrame(
        np.ones((4, 2)), columns=all_navigation, index=nodes)
    # temporary data nodal_energy_totals

    p_set = shipping_hydrogen_share * \
        nodal_energy_totals.loc[nodes, all_navigation].sum(
            axis=1) * 1e6 * efficiency / 8760
    ...
    if shipping_hydrogen_share < 1:
        ...
        co2 = shipping_oil_share * nodal_energy_totals.loc[nodes, all_navigation].sum(
        ).sum() * 1e6 / 8760 * costs.at["oil", "CO2 intensity"]
