...
def clean_data(tag_substation="transmission", threshold_voltage=35000):
    ...
    # Load raw data lines
    df_lines = gpd.read_file(raw_outputfile_partial + "_lines" + ".geojson").set_crs(
    ...
    # Load raw data lines
    df_cables = gpd.read_file(raw_outputfile_partial + "_cables" + ".geojson").set_crs(
        epsg=4326, inplace=True
    )
    ...
    df_all_lines = df_all_lines[
        df_all_lines["geometry"].map(lambda g: len(g.boundary) >= 2)
    ]
