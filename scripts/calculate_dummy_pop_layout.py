...
def calculate_dummy_pop_layout(n, inhabitants):
    ...
    pop_layout_eur = pd.read_csv(
        snakemake.input.clustered_pop_layout, index_col=0)

    # Create new pop_layout for morocco
    pop_layout_dummy = pd.DataFrame(
        columns=pop_layout_eur.columns, index=n.buses.index)
    ...
    pop_layout_dummy['total'] = pop_layout_dummy['urban'] + \
        pop_layout_dummy['rural']

    # Add country and fraction of nodes population to countries total population
    pop_layout_dummy['ct'] = 'MA'
    pop_layout_dummy['fraction'] = 1 / \
        len(pop_layout_dummy[pop_layout_dummy['ct'] == 'MA'].index)
