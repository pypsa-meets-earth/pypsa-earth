...
def process_hydropower_data(excel_path, hydro, sheet_name):
    ...
    if "name" not in excel_data.columns:
        raise KeyError(
            f"La colonna 'name' non è presente nei dati Excel. Colonne trovate: {list(excel_data.columns)}"
        )
    ...
    excel_columns_to_keep = [
        col
        for col in filtered_excel.columns
...
def create_time_series(final_data, hydro):
    ...
    monthly_columns = [
        f"{month.lower()}_capacity_factor_normal" for month in days_in_month.keys()
    ]
    ...
    if missing_columns:
        raise ValueError(
            f"Le seguenti colonne mancano in final_data: {missing_columns}"
        )
