...
def calculate_renewable_capital_cost(
    ...
    capital_cost_tech : float
        ...
        For non-offshore carriers this equals ``capital_cost``. Storing this on
        the network lets the per-horizon re-cost recover the frozen connection
        cost as ``capital_cost - capital_cost_tech`` without needing the
        profile geometry again.
    """
...
def update_electricity_costs(
    ...
    a horizon-specific ``costs`` table. It is intended to be called at the very
    start of ``prepare_network`` (before any opts-based cost scaling).

    The formulas below mirror exactly those used at build time in
    ``add_electricity`` and ``add_extra_components`` so the build and re-cost
    paths cannot diverge. Attributes that do NOT depend on the cost vintage are
    deliberately left untouched:
      ...
      * transmission ``efficiency`` (set from ``sector.transmission_efficiency``,
        not from the cost table).

    Offshore wind capital cost includes a distance-dependent grid connection
    cost that is treated as frozen infrastructure across horizons. It is
    recovered as ``capital_cost - capital_cost_tech`` (the latter stored at
    build time, see :func:`calculate_renewable_capital_cost`), so no profile
    geometry is needed here; only the technology component is re-costed.
    ...
    # --- Transmission lines and DC links (mirrors update_transmission_costs) ---
    ...
    for carrier in gens.carrier.dropna().unique():
        ...
        if carrier == "ror":
            # Run-of-river hydro: mirrors attach_hydro (no marginal_cost in
            # the cost table for ror).
            _assign(
                n.generators,
                is_carrier,
            ...
            # Wind/solar/csp: all cost attributes refreshed from horizon table.
            # For offshore wind, geometry (average_distance, underwater_fraction)
            # is stored on generators at build time; unit costs come from the
            # new table, so both the technology and connection parts are exact.
            if supcarrier == "offwind":
            ...
            # Conventional generators. marginal_cost is carrier-uniform at build
            # time, so refresh it for all units of the carrier. capital_cost,
            # efficiency and lifetime are only meaningful (and only refreshed)
            # for the extendable units; existing units keep their plant-specific
            # efficiency/lifetime and zero relevance capital_cost.
            _assign(
                n.generators,
                is_carrier,
    ...
    if not storage.empty:
        # PHS (existing): capital_cost + round-trip efficiency split (sqrt)
        _assign(
            n.storage_units,
            storage.carrier == "PHS",
        ...
        # Battery: existing (non-extendable) and extendable StorageUnits share
        # the carrier but were costed differently at build time.
        ...
        # Extendable H2 StorageUnit (only present if modelled as StorageUnit)
        _assign(
            n.storage_units,
            storage.carrier == "H2",
    ...
    if not links.empty:
        ...
        # H2 pipelines: capital_cost scales with length; efficiency is set from
        # the transmission-efficiency config (not the cost table) so it is left
        # untouched here.
