...
def plot_water_networks(
    regions_onshore_aqueduct,
    ...
    shorelines_natura.to_crs(epsg=4326).plot(
        ax=ax, color="blue", label="Clipped Shoreline"
    )
    ...
    plt.savefig(
        snakemake.output.water_network, dpi=300, bbox_inches="tight"
...
def plot_altitude_profile(df, resolution=1000):
    ...
    inv = (
        1.0 / k
    )  # scaling factor for effective_increment_m and head matching prepare_water_network
    factors = {
        "distance_m": k,
        # "elevation_m": inv,
        "head_clamped_m": inv,
    ...
    if "effective_increment_m" in df:
        ...
        plt.bar(
            ...
            df["effective_increment_m"].iloc[1:].fillna(0.0),
            width=w,
            align="edge",
            alpha=0.4,
        ...
        plt.gca().text(
            0.99,
            0.05,
            f"Σ added head = {total_added:.1f} m",
            transform=plt.gca().transAxes,
            ha="right",
            va="bottom",
            bbox=dict(
                boxstyle="round,pad=0.3", facecolor="white", alpha=0.7, linewidth=0
            ),
        )
    ...
    path_to_save = (
        ...
        / "plots"
        / "desalination"
        / f"{df['line_name'].iloc[0]}_profile.png"
    )
...
if __name__ == "__main__":
    ...
    regions_onshore_aqueduct_desalination = gpd.read_file(
        snakemake.input.regions_onshore_aqueduct_desalination
    )
    ...
    water_pipes_profiles = read_csv_nafix(
        snakemake.input.water_pipes_profiles, index_col=0
    )

    # Convert the 'centroid' column from WKT to geometry
    regions_onshore_aqueduct_desalination["centroid"] = (
        regions_onshore_aqueduct_desalination["centroid"].apply(wkt.loads)
    )
    regions_onshore_aqueduct_desalination = (
        regions_onshore_aqueduct_desalination.set_geometry("centroid", crs="EPSG:4326")
    )
