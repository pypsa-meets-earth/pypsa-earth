...
if __name__ == "__main__":
    ...
    if snakemake.wildcards.technology.startswith("hydro"):
        ...
        if "grid_codes" in copernicus:
            ...
            excluder.add_raster(
                paths.copernicus, codes=codes, invert=True, crs=3035)
        if copernicus.get("distance", 0.) > 0.:
            codes = copernicus["distance_grid_codes"]
            buffer = copernicus["distance"]
            excluder.add_raster(paths.copernicus, codes=codes,
                                buffer=buffer, crs=3035)
        ...
        if "max_depth" in config:
            ...
            excluder.add_raster(
                paths.gebco, codes=func_depth, crs=4236, nodata=-1000)
        ...
        if snakemake.wildcards.technology.startswith("offwind"):
            ...
            ds['underwater_fraction'] = xr.DataArray(
                underwater_fraction, [buses])
