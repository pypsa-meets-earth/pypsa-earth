...
import numpy as np
import pandas as pd
from _helpers import _save_to_geojson, _to_csv_nafix, configure_logging
...
def integrate_lines_df(df_all_lines):
    ...
    if df_all_lines["cables"].dtype != int:
        ...
        df_all_lines.loc[
            ...
            # e.g. in the DK data
            | (df_all_lines["cables"] == "ground") | df_all_lines["cables"].isna(),
            "cables",
        ] = "0"
    ...
    df_all_lines.loc[
        ...
        # e.g. in the JP data
        | (df_all_lines["circuits"] == "2/3") | (df_all_lines["circuits"] == 0),
        "circuits",
    ] = 1
    ...
    if df_all_lines["circuits"].dtype != int:
        ...
        df_all_lines.loc[
            (df_all_lines["circuits"] == "1/3") | df_all_lines["circuits"].isna(),
            "circuits",
        ] = "0"
