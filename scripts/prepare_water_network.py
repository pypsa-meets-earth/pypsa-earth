...
def add_pipeline_hydraulics(row):
    ...
    eta_pump, eta_motor = (
        watersupply_cfg["efficiencies"]["eta_pump"],
        watersupply_cfg["efficiencies"]["eta_motor"],
    )
    eta_sys = eta_pump * eta_motor
    lambda_f = watersupply_cfg["pipeline_hydraulics"]["friction_factor_lambda"]
    mass_flow_rate = watersupply_cfg["pipeline_hydraulics"][
        "mass_flow_rate_kgs"
    ]  # kg/s, Considered mass flow rate for 1000 MW Electrolyzer
    ...
    n_pumping_stations = number_of_pumping_stations(
        ...
        p_max_pipeline=watersupply_cfg["pipeline_hydraulics"][
            "p_max_pipeline_bar"
        ],  # nominal pressure of the pipeline in bar
    )
    ...
    invest_pumping_station = pump_station_cost_kw(
        ...
        a=watersupply_cfg["costs"]["pump_station"][
            "cost_factor_a"
        ],  # 15570 # 35768 with inflation 2% 1981 till 2023
    ...
    C_pipes = watersupply_cfg["costs"]["pipeline"]["pipe_mat_factor"] * (
        d_mm ** watersupply_cfg["costs"]["pipeline"]["pipe_mat_exponent"]
    )  # €/km material
    f_inst = watersupply_cfg["costs"]["pipeline"]["inst_factor"] * (
        d_mm ** watersupply_cfg["costs"]["pipeline"]["inst_exponent"]
    )  # installation factor
