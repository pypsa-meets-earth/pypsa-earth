...
def _add_land_use_constraint(n):
    ...
    for carrier in ['solar', 'onwind', 'offwind-ac', 'offwind-dc']:
        existing = n.generators.loc[n.generators.carrier == carrier, "p_nom"].groupby(
            n.generators.bus.map(n.buses.location)).sum()
...
def _add_land_use_constraint_m(n):
    ...
    for carrier in ['solar', 'onwind', 'offwind-ac', 'offwind-dc']:
        ...
        ind = list(set([i.split(sep=" ")[0] + ' ' + i.split(sep=" ")[1]
        ...
        for p_year in previous_years:
            ind2 = [i for i in ind if i + " " +
                    carrier + "-" + p_year in existing.index]
            sel_current = [i + " " + carrier +
                           "-" + current_horizon for i in ind2]
            sel_p_year = [i + " " + carrier + "-" + p_year for i in ind2]
            n.generators.loc[sel_current, "p_nom_max"] -= existing.loc[sel_p_year].rename(
